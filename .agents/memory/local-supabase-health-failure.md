...
**How to apply:** Treat `npm run validate:db` as blocked until the local container health check works in a Docker-capable environment. Do not substitute partial/manual migration application for the required clean reset, pgTAP, signed-client isolation, seed idempotency, type comparison, lint, tests, and build gate. Leave the local stack stopped after an unsuccessful attempt.