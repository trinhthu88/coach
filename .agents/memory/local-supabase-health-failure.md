...
**How to apply:** Treat `npm run validate:db` as blocked until the local container health check works in a Docker-capable environment. The pinned local run reconfirmed the failure as `LegacyHealthCheckTimeoutError` before project migrations replayed. Do not use `db reset --db-url` or partial/manual migration application as a workaround: a failed health-gated reset can leave the local database without the project or storage schema. Do not substitute this for the required clean reset, pgTAP, signed-client isolation, seed idempotency, type comparison, lint, tests, and build gate. Leave the local stack stopped after an unsuccessful attempt.