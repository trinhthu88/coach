...
export default function CoacheePeerBookSession() {
    ...
    (async () => {
      try {
        ...
        if (user) {
          ...
          const [{ data: mySess }, { data: myMentoring }, { data: myPeer }] = await Promise.all([
            supabase
              .from("sessions")
            ...
            supabase
              ...
              .select("start_time, duration_minutes, status, peer_provider_id, peer_receiver_id")
          ...
          setBookerBusy([...toBusy(mySess), ...toBusy(myMentoring), ...toBusy(myPeer)]);
        }
        setLoading(false);
      } catch (err) {
        console.error("Failed to load peer-practice booking data:", err);
        setLoadError(true);
        setLoading(false);
      }
    })();
  }, [partnerId, user, retryKey]);
  ...
  const startOptions = useMemo(() => {
    ...
    return computeStartOptions({ dateKey: dateKey(selectedDate), slots, durationMinutes: duration, busy: bookerBusy });
  ...
  const handleBook = async () => {
    ...
    if (error) {
      if ((error as { code?: string }).code === "23505") {
        ...
        toast.error(t("coacheePeerPractice.bookSession.toast.slotTaken"));
        return;
      }
      if ((error as { code?: string }).code === "42501") {
        toast.error(t("coacheePeerPractice.bookSession.toast.notEligible"));
        return;
      }
      return toast.error(getFriendlyErrorMessage(error, t));
    }
    toast.success(t("coacheePeerPractice.bookSession.toast.success"));
    // No dedicated session-detail view for this table yet (see task notes) —
    // land on the generic Sessions list, matching what the coach peer pool
    // does today (BookSession.tsx navigates to /sessions for mode=peer too).
    navigate("/sessions");
  };
  ...
  if (loadError) {
    return (
      <Card className="p-12 text-center">
        <h2 className="text-xl font-semibold">{t("coacheePeerPractice.bookSession.loadError.title")}</h2>
        <p className="mt-2 text-sm text-muted-foreground">{t("coacheePeerPractice.bookSession.loadError.body")}</p>
        <Button variant="outline" className="mt-6" onClick={() => setRetryKey((k) => k + 1)}>
          {t("coacheePeerPractice.bookSession.loadError.retry")}
        </Button>
      </Card>
    );
  }
  if (!partner) {
    return (
      <Card className="p-12 text-center">
        <h2 className="text-xl font-semibold">{t("coacheePeerPractice.bookSession.partnerNotFound.title")}</h2>
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <ChevronLeft className="h-4 w-4" /> {t("coacheePeerPractice.bookSession.back")}
      </Link>
        ...
        <Card className="space-y-6 p-4 sm:p-6">
          <div>
            <h1 className="font-display text-[1.5rem] leading-[1.1] tracking-tight sm:text-[1.9rem]">
              {t("coacheePeerPractice.bookSession.title")}
            </h1>
            <p className="text-sm text-muted-foreground">{t("coacheePeerPractice.bookSession.subtitle")}</p>
          </div>

          <Step number={1} label={t("coacheePeerPractice.bookSession.steps.duration")}>
            <div className="mt-3 grid grid-cols-3 gap-3">
              {DURATIONS.map((d) => (
                ...
                >
                  {t("coacheePeerPractice.bookSession.steps.durationOption", { n: d })}
                </button>
              ))}
            </div>
          </Step>

          <Step number={2} label={t("coacheePeerPractice.bookSession.steps.date")}>
              ...
              <div className="grid min-w-0 flex-1 grid-cols-7 gap-1 sm:gap-2">
                {week.map((d) => {
                  ...
                  return (
                    ...
                    >
                      <div className="text-[9px] font-bold uppercase tracking-widest opacity-80 sm:text-[10px]">{format(d, "EEE")}</div>
          ...
          <Step
            number={3}
            label={t("coacheePeerPractice.bookSession.steps.time", { timezone: Intl.DateTimeFormat().resolvedOptions().timeZone })}
          >
            {!selectedDate ? (
              <p className="mt-3 text-sm text-muted-foreground">{t("coacheePeerPractice.bookSession.pickDatePrompt")}</p>
            ) : startOptions.length === 0 ? (
              <p className="mt-3 text-sm text-muted-foreground">{t("coacheePeerPractice.bookSession.noWindowAvailable", { duration })}</p>
            ) : (
              <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                {startOptions.map((o) => (
                  <button
                    ...
                    aria-label={t("coacheePeerPractice.bookSession.slotAriaLabel", {
          ...
          <div>
            <Label htmlFor="topic" className="text-xs font-bold uppercase tracking-widest text-muted-foreground">
              {t("coacheePeerPractice.bookSession.topicLabel")}
            </Label>
            <Textarea
              id="topic"
              value={topic}
              onChange={(e) => setTopic(e.target.value)}
              placeholder={t("coacheePeerPractice.bookSession.topicPlaceholder")}
              className="mt-2"
              rows={3}
            />
          </div>
          ...
          <div className="flex flex-col items-start justify-between gap-3 border-t pt-4 sm:flex-row sm:items-center">
            ...
            <Button onClick={handleBook} disabled={!canSubmit || submitting} size="lg" className="shadow-glow">
              ...
              {t("coacheePeerPractice.bookSession.confirmButton")}
            </Button>
          </div>
        </Card>
      </div>
  ...
  return (
    <div>
      <div className="flex items-center gap-2">
        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-primary-soft text-xs font-bold text-primary">{number}</span>
