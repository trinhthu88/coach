...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: t("resetPassword.toast.error.title"),
        description: err instanceof Error ? err.message : t("resetPassword.toast.error.genericDescription"),
        variant: "destructive",
      });
    } finally {
      setLoading(false);
    ...
    <div className="relative flex min-h-screen items-center justify-center bg-gradient-subtle p-6">
      ...
      <Card className="w-full max-w-md p-8 sm:p-10">
        ...
        <h1 className="font-display text-3xl font-light tracking-tight text-secondary">
          {t("resetPassword.titleLead")} <em className="text-primary">{t("resetPassword.titleEmphasis")}</em>
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {ready ? t("resetPassword.subtitleReady") : t("resetPassword.subtitleValidating")}
        </p>
