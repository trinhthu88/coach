...
export function ProgrammeMetricCards({
  ...
  return (
    <div data-testid="programme-kpis" className="mt-4 grid gap-3.5 [grid-template-columns:repeat(auto-fit,minmax(150px,1fr))]">
      <ProfileKpi label={text("overallCompletion")} value={formatPercent(canonicalCompletionPct(facts.full_completion_pct))} color={NAVY} />
      ...
      <ProfileKpi label={text("goalProgress")} value={goalProgressText(goalProgressState(engagement), text)} color={TEAL} />
      <ProfileKpi
