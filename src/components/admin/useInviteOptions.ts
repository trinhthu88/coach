...
export interface InviteCohortOpt { id: string; name: string; programme_id: string | null; organization_id: string | null }
...
export function useInviteOptions(enabled: boolean) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from("cohorts").select("id, name, programme_id, organization_id").order("name"),
