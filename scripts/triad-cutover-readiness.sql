...
\echo '== 3. Blocking data inconsistencies (cutover section 1) — every list must be empty'
SELECT 'group slots inconsistent with enrollments' AS check, g.id AS record_id
FROM public.triad_groups g
...
FROM public.triad_reflections r
WHERE r.enrollment_id IS NULL AND NOT public.is_historical_ownership_retired('triad_reflections', r.id)
UNION ALL
