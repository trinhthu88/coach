...
This is the working reference for reviewing every `vi/*.json` batch produced during Phase 5 of the i18n build. Bring future batches back with this guide attached so the register stays consistent across the app rather than drifting page to page.

## Locked decision

**Bilingual mix**, not full translation: *coach*, *coaching*, and *session* stay as English loanwords in-product. Everything else is translated into natural Vietnamese. This matches how Vietnamese banking/finance L&D audiences (the target — Seabank, Techcombank-style clients) actually read premium coaching products; a fully-Vietnamese UI can read as less premium to this specific audience, not more accessible.
...
English (source, unchanged) alongside the Vietnamese localization. This is the **approved, human-reviewed** text for the coachee onboarding namespace — use it verbatim, do not machine-retranslate.
...
`auth.json`, `dashboard.json`, `journey.json`, `landing.json`, and `sessions.json` were translated before this guide existed and used "huấn luyện viên" for coach and "buổi học" for session throughout. They've since been retrofitted to the locked bilingual-mix rule (coach/coaching/session stay English). `admin.json` and `common.json` didn't reference these terms and needed no changes. These five files are still machine-translated pending human review overall (see each file's `__notice` key) — only the coach/coaching/session terminology has been brought in line with this guide; the rest of the copy hasn't been re-reviewed against the register notes below.

## Next steps

- Coach and sponsor onboarding steps are now in `src/locales/{en,vi}/onboarding.json` (under the `"coach"` and `"sponsor"` keys) — but only **machine-translated** against this glossary, not yet human-reviewed like the coachee section above. They need the same approved pass before shipping to real users; paste or upload that JSON content here for revision against this glossary rather than starting from scratch.
- Small wording calls made here (e.g. "trang tổng quan" for dashboard) set precedent for the whole app — confirm before extending elsewhere.
