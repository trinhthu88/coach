-- Each attribution-based progress branch must consume only the source table
-- it can resolve. Without these filters, every attribution is emitted by
-- every UNION branch and coaching activity is counted multiple times.
