...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  ...
  activity as (
    ...
    union all select 'peer_coaching'::public.programme_module_type,enrollment_id,status::text,start_time::date from public.peer_sessions
    union all select 'peer_coaching'::public.programme_module_type,enrollment_id,status::text,start_time::date from public.coachee_peer_sessions
    union all select 'mentoring'::public.programme_module_type,enrollment_id,status::text,start_time::date from public.mentoring_sessions
    union all select 'triads'::public.programme_module_type,coach_enrollment_id,status::text,start_time::date from public.triad_sessions where coach_enrollment_id is not null
    union all select 'triads'::public.programme_module_type,coachee_enrollment_id,status::text,start_time::date from public.triad_sessions where coachee_enrollment_id is not null
    union all select 'triads'::public.programme_module_type,observer_enrollment_id,status::text,start_time::date from public.triad_sessions where observer_enrollment_id is not null
    union all select 'training'::public.programme_module_type,enrollment_id,case when completed_at is null then 'confirmed'::text else 'completed'::text end,coalesce(completed_at,created_at)::date from public.training_progress
  ), counts as (
