-- Keep the Leader Detail experience payload bounded to the fields its
-- consumers actually use. The previous implementation recalculated weekly
-- activity and engagement in the same request even though Leader Detail now
-- reads canonical checkpoints and metadata for those sections. That made the
-- optional enrichment compete with the two canonical requests and hit the
-- statement timeout under normal browser load.

CREATE OR REPLACE FUNCTION public.sponsor_canonical_leader_experience_base(
