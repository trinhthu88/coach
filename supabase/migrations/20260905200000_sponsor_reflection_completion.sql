-- Spec: reflection completion % belongs on the live sponsor dashboard,
-- alongside skill card / quiz / triad / prompt completion — it was never
-- added when the reflection module was built. Confidence (already
-- report-only via sponsor_confidence_trend()) is unaffected.
--
-- DROP first: CREATE OR REPLACE cannot add/remove/reorder a function's
-- RETURNS TABLE columns (SQLSTATE 42P13) — adding reflection_completion_pct
-- requires an explicit drop, not just a replace.
