...
--    COMPLETED Triad session of the programme's requirement-scoped groups
--    (optionally within a session-date window). Submitted: that member's
--    reflection exists. Rows per Training week of the requirement, plus one
--    total row (is_total).
-- ----------------------------------------------------------------------------
...
AS $$
  WITH expected AS (
    ...
    WHERE d.programme_id = p_programme_id
