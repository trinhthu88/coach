-- Reconcile organization counts with the cohort-summary population.
--
-- sponsor_metric_rows() intentionally omits below-threshold cohorts so their
-- detailed metrics remain suppressed.  The cohort summary still includes
-- those cohorts as aggregate-only suppressed rows.  Count the organization
-- cohort/enrollment population from the same sponsor-visible cohort sizes,
-- while keeping all detailed metrics on the threshold-aware metric rows.
...
AS $function$
  WITH sponsor AS (
    ...
    SELECT c.id AS cohort_id, count(e.id)::integer AS enrollment_count
    ...
    SELECT
      (SELECT coalesce(sum(cs.enrollment_count), 0)::integer
       FROM cohort_sizes cs) AS enrollment_count,
