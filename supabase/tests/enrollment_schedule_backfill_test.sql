...
select results_eq(
 ...
 $$values (4,1,2,1)$$, 'batch backfills valid rows and isolates invalid and partial rows');
...
select results_eq(
 $$select processed,succeeded,unresolved,skipped from public.backfill_enrollment_schedule_snapshots()$$,
 $$values (4,1,1,2)$$, 'corrected configuration succeeds on retry');
...
select set_config('request.jwt.claim.sub','aa000000-0000-0000-0000-000000000002',true);
delete from public.enrollment_schedule_backfill_audit where reason='partial_snapshot';
select lives_ok($$select public.assert_enrollment_schedule_backfill_ready()$$,'readiness succeeds after all audits are cleared');
select results_eq(
 $$select processed,succeeded,unresolved,skipped from public.backfill_enrollment_schedule_snapshots()$$,
 $$values (4,0,0,4)$$, 'repeated clean run is idempotent');
select * from finish();
rollback;