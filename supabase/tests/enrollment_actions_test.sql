...
insert into public.coachee_goals(id,coachee_id,enrollment_id,title) values
 ('a5000000-0000-4000-8000-000000000053','a5000000-0000-4000-8000-000000000003','a5000000-0000-4000-8000-000000000032','Booking gate goal');
...
select throws_ok($$insert into public.enrollment_actions(enrollment_id,owner_user_id,title,goal_id,milestone_id,due_date,source_activity_type,source_activity_id) values('a5000000-0000-4000-8000-000000000031','a5000000-0000-4000-8000-000000000001','Invalid','a5000000-0000-4000-8000-000000000052','a5000000-0000-4000-8000-000000000060','2026-03-01','coaching','a5000000-0000-4000-8000-000000000041')$$,'42501','Action milestone must belong to the action goal','reject inconsistent goal/milestone link');
select throws_ok($$insert into public.enrollment_actions(enrollment_id,owner_user_id,title,goal_id,due_date) values('a5000000-0000-4000-8000-000000000031','a5000000-0000-4000-8000-000000000001','No source','a5000000-0000-4000-8000-000000000051','2026-03-01')$$,'23514','New post-session actions require their source activity','reject a new action with no source activity');
select throws_ok($$insert into public.enrollment_actions(enrollment_id,owner_user_id,title,due_date,source_activity_type,source_activity_id) values('a5000000-0000-4000-8000-000000000031','a5000000-0000-4000-8000-000000000001','No goal','2026-03-01','coaching','a5000000-0000-4000-8000-000000000041')$$,'23514','New post-session actions require a goal and due date','reject a new action with no goal');
select throws_ok($$insert into public.enrollment_actions(enrollment_id,owner_user_id,title,goal_id,source_activity_type,source_activity_id) values('a5000000-0000-4000-8000-000000000031','a5000000-0000-4000-8000-000000000001','No date','a5000000-0000-4000-8000-000000000051','coaching','a5000000-0000-4000-8000-000000000041')$$,'23514','New post-session actions require a goal and due date','reject a new action with no due date');
select throws_ok($$insert into public.enrollment_actions(enrollment_id,owner_user_id,title,goal_id,due_date,source_activity_type,source_activity_id) values('a5000000-0000-4000-8000-000000000031','a5000000-0000-4000-8000-000000000001','Other goal','a5000000-0000-4000-8000-000000000053','2026-03-01','coaching','a5000000-0000-4000-8000-000000000041')$$,'42501','Action goal must belong to the action enrollment','reject a goal of another enrollment');
select set_config('request.jwt.claim.sub','a5000000-0000-4000-8000-000000000002',true);
select lives_ok($$select public.save_enrollment_activity_actions('a5000000-0000-4000-8000-000000000031','coaching','a5000000-0000-4000-8000-000000000041','[{"id":"a5000000-0000-4000-8000-000000000070","title":"Ask for feedback","goal_id":"a5000000-0000-4000-8000-000000000051","milestone_id":"a5000000-0000-4000-8000-000000000060","due_date":"2026-03-01","status":"open"}]')$$,'actual provider can save source-linked actions');
...
select lives_ok($$select public.save_enrollment_activity_actions('a5000000-0000-4000-8000-000000000031','coaching','a5000000-0000-4000-8000-000000000041','[{"id":"a5000000-0000-4000-8000-000000000070","title":"Ask for feedback","status":"completed","goal_id":"a5000000-0000-4000-8000-000000000051","milestone_id":"a5000000-0000-4000-8000-000000000060","due_date":"2026-03-01"}]')$$,'repeated save updates stable action ID');
...
select throws_ok($$select public.save_enrollment_activity_actions('a5000000-0000-4000-8000-000000000032','coaching','a5000000-0000-4000-8000-000000000042','[{"id":"a5000000-0000-4000-8000-000000000070","title":"Steal action","goal_id":"a5000000-0000-4000-8000-000000000053","due_date":"2026-03-01"}]')$$,'42501','Action ID belongs to another source or enrollment','provider cannot transfer another source action by ID');
select set_config('request.jwt.claim.sub','a5000000-0000-4000-8000-000000000003',true);
...
select is((select action_items from public.sessions where id='a5000000-0000-4000-8000-000000000041'),'[{"text":"Legacy linked action","done":true,"goal_id":"a5000000-0000-4000-8000-000000000051","milestone_id":"a5000000-0000-4000-8000-000000000060","due_date":"2026-03-01"},{"text":"Broken date action","due_date":"not-a-date"}]'::jsonb,'backfill preserves original legacy JSON including unresolved items');
select set_config('request.jwt.claim.sub','a5000000-0000-4000-8000-000000000001',true);
set local role authenticated;
select lives_ok($$select public.save_enrollment_activity_actions('a5000000-0000-4000-8000-000000000031','coaching','a5000000-0000-4000-8000-000000000041','[{"id":"a5000000-0000-4000-8000-000000000070","title":"Learner update","goal_id":"a5000000-0000-4000-8000-000000000051","due_date":"2026-03-01"}]')$$,'authenticated learner may save their activity actions');
select throws_ok($$select public.save_enrollment_activity_actions('a5000000-0000-4000-8000-000000000031','coaching','a5000000-0000-4000-8000-000000000041','[{"id":"a5000000-0000-4000-8000-000000000070","title":"Duplicate","goal_id":"a5000000-0000-4000-8000-000000000051","due_date":"2026-03-01"},{"id":"a5000000-0000-4000-8000-000000000070","title":"Duplicate","goal_id":"a5000000-0000-4000-8000-000000000051","due_date":"2026-03-01"}]')$$,'P0001','Duplicate action ID','duplicate IDs reject the entire save');
...
select throws_ok($$select public.save_enrollment_activity_actions('a5000000-0000-4000-8000-000000000031','coaching','a5000000-0000-4000-8000-000000000041','[{"title":"bad","goal_id":"not-a-uuid","due_date":"2026-03-01"}]')$$,'P0001','Action goal ID is invalid','reject malformed goal ID');
select throws_ok($$select public.save_enrollment_activity_actions('a5000000-0000-4000-8000-000000000031','coaching','a5000000-0000-4000-8000-000000000041','[{"title":"bad","goal_id":"a5000000-0000-4000-8000-000000000051","due_date":"2026-99-99"}]')$$,'P0001','Action UUID or date is invalid','reject malformed date');
select throws_ok($$select public.save_enrollment_activity_actions('a5000000-0000-4000-8000-000000000031','coaching','a5000000-0000-4000-8000-000000000041','[{"title":"bad","goal_id":"a5000000-0000-4000-8000-000000000051","due_date":"2026-03-01","status":"unknown"}]')$$,'P0001','Action status is invalid','reject unknown status');
select throws_ok($$select public.save_enrollment_activity_actions('a5000000-0000-4000-8000-000000000031','coaching','a5000000-0000-4000-8000-000000000041','[{"title":"bad","goal_id":"a5000000-0000-4000-8000-000000000051","due_date":"2026-03-01","unexpected":true}]')$$,'P0001','Unknown action field','reject unknown fields');
